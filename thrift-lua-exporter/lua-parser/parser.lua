...
local function tagC (tag, patt)
  return Ct(Cg(Cp(), "pos") * Cg(Cc(tag), "tag") * patt * Cg(Cp(), "epos"))
end
...
local function insertIndex (t, index)
  return { tag = "Index", pos = t.pos, epos = t.epos, [1] = t, [2] = index }
end

local function markMethod(t, method)
  if method then
    return { tag = "Index", pos = t.pos, epos = t.epos, is_method = true, [1] = t, [2] = method }
  end
  return t
end
...
local function makeIndexOrCall (t1, t2)
  if t2.tag == "Call" or t2.tag == "Invoke" then
    local t = { tag = t2.tag, pos = t1.pos, epos = t1.epos, [1] = t1 }
    for k, v in ipairs(t2) do
      table.insert(t, v)
    end
    return t
  end
  return { tag = "Index", pos = t1.pos, epos = t1.epos, [1] = t1, [2] = t2[1] }
end
