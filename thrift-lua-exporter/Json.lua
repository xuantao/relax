...
local function grok_object(self, text, start, options)
   ...
   while i <= text_len do
      ...
      local new_val, new_i1 = grok_one(self, text, i, options)
...
local function json_string_literal(value, options)
   ...
   if options and options.stringsAreUtf8 then
      --
...
function JsonEncode(value, options, etc)
    if options == true then
        ...
        return JSON:encode(value, etc, options)
    end
end
